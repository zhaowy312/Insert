...
class Runner:
    ...
    def predict(self, obs_dict, requires_grad=False, display=False, num_inference_steps=3):
        if not requires_grad:
            self.model.eval()
            with torch.no_grad():
                return self._predict_forward(obs_dict, display, num_inference_steps)
        else:
            return self._predict_forward(obs_dict, display, num_inference_steps)

    def _extract_features(self, obs_dict, display=False):
    ...
    def _predict_forward(self, obs_dict, display=False, num_inference_steps=3):
        obs_features = self._extract_features(obs_dict, display)
