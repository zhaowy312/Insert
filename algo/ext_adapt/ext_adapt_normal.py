    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            with torch.no_grad():
                ...
                # --- [优化 FPS 的 DAgger 调度] ---
                warmup_steps = 2e6  # 前 200 万步纯 Teacher 演示，不执行学生模型推理
                decay_steps  = 8e6  # 200万~1000万步期间，平滑移交控制权
                
                if self.agent_steps < warmup_steps:
                    beta = 1.0
                else:
                    progress = (self.agent_steps - warmup_steps) / decay_steps
                    beta = max(0.0, 1.0 - progress)

                use_teacher = torch.rand(1).item() < beta

                # 直接回归模式下，推理开销极低（单次前向传播），
                # 所以无论是否使用 teacher 动作，都做 student 推理以：
                # 1. 更新 running mean/std 统计量
                # 2. 验证 student 输出是否合理
                student_actions, _ = self.student.predict(student_dict, requires_grad=False)
    ...
    def train_epoch(self):
        ...
        for _ in range(0, self.mini_epochs_num):
            for i in range(len(self.storage)):
                ...
                # 【修复】multi_gpu 同步在 optim.step() 之前
                if self.multi_gpu:
                    all_grads_list = []
